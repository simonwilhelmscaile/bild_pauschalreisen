...
# Import field utilities from blog shared
try:
    from services.blog.shared.field_utils import iter_html_fields
except ImportError:
    try:
        from shared.field_utils import iter_html_fields
    except ImportError as e:
        iter_html_fields = None
        logger.warning(f"Stage 5 field_utils import failed: {e}")
...
try:
    # Use absolute import to get the blog's prompt_loader (not context's)
    from services.blog.shared.prompt_loader import load_prompt
    _PROMPT_LOADER_AVAILABLE = True
    print("[stage5] Using services.blog.shared.prompt_loader")
except ImportError as e:
    print(f"[stage5] Absolute import failed: {e}")
    try:
        # Fallback for relative import
        from ..shared.prompt_loader import load_prompt
        _PROMPT_LOADER_AVAILABLE = True
        print("[stage5] Using relative ..shared.prompt_loader")
    except ImportError as e2:
        print(f"[stage5] Relative import failed: {e2}")
        try:
            # Last fallback for direct script execution
            from shared.prompt_loader import load_prompt
            _PROMPT_LOADER_AVAILABLE = True
            print("[stage5] Using shared.prompt_loader (fallback)")
        except ImportError:
            _PROMPT_LOADER_AVAILABLE = False
            print("[stage5] No prompt_loader available")


def _get_internal_links_prompt(links_text: str, sections_text: str, keyword: str = "") -> str:
...
- ONLY link to pages relevant to the article topic "{keyword}" — do NOT cross-link unrelated product categories
...
class InternalLinker:
    """
    Embeds internal links into article content using core GeminiClient.
    """

    def __init__(self, api_key: Optional[str] = None):
        """Initialize with core GeminiClient."""
        if GeminiClient is None:
            raise ImportError("core.gemini_client not available")

        if ServiceType is not None:
            self._client = GeminiClient(service_type=ServiceType.BLOG, api_key=api_key)
        else:
            self._client = GeminiClient(api_key=api_key)
        logger.info("InternalLinker initialized (using core GeminiClient)")
    ...
    async def run(self, input_data: Stage5Input) -> Stage5Output:
        ...
        embeddings = await self._get_embeddings(sections_text, link_pool, keyword=input_data.keyword)
        if not embeddings:
    ...
    def _build_link_pool(self, input_data: Stage5Input) -> List[LinkCandidate]:
        ...
        BLOCKED_PATTERNS = ['/app-welt/', '/produktberater']

        def is_blocked(url: str) -> bool:
            return any(pat in url.lower() for pat in BLOCKED_PATTERNS)

        # Normalize current href for comparison
        current_href = input_data.current_href.lower().strip("/")

        # Add category overview URLs first (highest priority per feedback)
        for url in input_data.sitemap_category_urls:
            rel_path = self._normalize_to_path(url)
            if rel_path and rel_path not in seen_paths and not is_blocked(url):
        ...
        for sibling in input_data.batch_siblings[:5]:
            ...
            if href and not self._is_current_article(href, current_href):
                ...
                if rel_path and rel_path not in seen_paths and not is_blocked(href):
                    candidates.append(LinkCandidate(
                        url=rel_path,
        ...
        for url in input_data.sitemap_blog_urls[:20]:
            rel_path = self._normalize_to_path(url)
            if rel_path and rel_path not in seen_paths and not self._is_current_article(url, current_href) and not is_blocked(url):
                candidates.append(LinkCandidate(
    ...
    async def _get_embeddings(self, sections_text: str, link_pool: List[LinkCandidate], keyword: str = "") -> List[LinkEmbedding]:
        ...
        prompt = _get_internal_links_prompt(links_text, sections_text, keyword=keyword)
        ...
        try:
            ...
            # Use core GeminiClient with schema (no grounding needed)
    ...
    def _validate_replacement(self, find_text: str, replace_text: str, valid_urls: Set[str]) -> Optional[str]:
        ...
        if not find_text or len(find_text.strip()) < 2:
            logger.debug(f"  Find text too short: '{find_text}'")
            return None
        ...
        # Normalize the path (remove trailing slashes, lowercase for comparison)
        url_path_normalized = url_path.rstrip('/').lower()

        # Validate URL is from our pool (flexible matching)
        url_found = None
        for valid_url in valid_urls:
            valid_normalized = valid_url.rstrip('/').lower()
            if url_path_normalized == valid_normalized:
                url_found = valid_url
                break
            # Also try matching just the slug (last segment)
            url_slug = url_path_normalized.split('/')[-1] if '/' in url_path_normalized else url_path_normalized
            valid_slug = valid_normalized.split('/')[-1] if '/' in valid_normalized else valid_normalized
            if url_slug and url_slug == valid_slug:
                url_found = valid_url
                break
        
        if not url_found:
            logger.debug(f"  URL not in pool: {url_path} (pool has {len(valid_urls)} URLs)")
            return None

        # Validate anchor text matches find_text (Gemini should use same text)
        # Compare stripped versions - be lenient here
        find_plain = find_text.strip()
        if anchor_plain.lower() != find_plain.lower():
            # Check if anchor is substring of find or vice versa
            if find_plain.lower() not in anchor_plain.lower() and anchor_plain.lower() not in find_plain.lower():
                logger.debug(f"  Anchor mismatch: '{anchor_plain}' vs '{find_plain}'")
                # Still allow - Gemini might shorten anchor

        # Return the matched path for consistent deduplication
        return url_found
    ...
    def _apply_embeddings(self, article: Dict[str, Any], embeddings: List[LinkEmbedding], valid_urls: Set[str]) -> int:
        ...
        for emb in embeddings:
            ...
            if find_text not in content:
                ...
                if find_lower in content_lower:
                    # Find the actual text with original casing
                    idx = content_lower.find(find_lower)
                    actual_find_text = content[idx:idx + len(find_text)]
                else:
                    logger.warning(f"  Text not found in {field}: '{find_text[:50]}...'")
                    continue

            # Find first UNPROTECTED occurrence
            safe_pos = self._find_first_unprotected(content, actual_find_text)
            if safe_pos == -1:
                logger.warning(f"  Text inside protected tag in {field}: '{actual_find_text[:30]}...'")
                continue

            # Validate replacement format and URL (more expensive - do last)
            url = self._validate_replacement(actual_find_text, replace_text, valid_urls)
            if not url:
                logger.warning(f"  Invalid replacement for '{actual_find_text[:30]}...': {replace_text[:60]}...")
                continue
            ...
            # Use actual_find_text for correct casing
            # Guard against collapsed word boundaries: ensure spaces exist
            # between the replacement and surrounding text
            before = content[:safe_pos]
            after = content[safe_pos + len(actual_find_text):]
            if before and not before[-1].isspace() and not replace_text[0:1].isspace() and before[-1] != '>':
                replace_text = ' ' + replace_text
            if after and not after[0:1].isspace() and not replace_text[-1:].isspace() and after[0:1] != '<':
                replace_text = replace_text + ' '
            article[field] = before + replace_text + after
            applied += 1
            logger.info(f"  Embedded link in {field}: {url}")
...
async def run_stage_5(input_data: Dict[str, Any], api_key: Optional[str] = None) -> Dict[str, Any]:
    ...
    Raises:
        ...
        ImportError: If core.gemini_client is not available
    """
