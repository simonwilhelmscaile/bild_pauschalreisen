...
class CompanyContext(BaseModel):
    ...
    @field_validator("industry", "description", "company_name", "company_url", "tone", mode="before")
    @classmethod
    def convert_none_to_empty_string(cls, v):
        """Convert None to empty string for string fields."""
        return v if v is not None else ""

    @field_validator("products", "competitors", "pain_points", "value_propositions", "use_cases", "content_themes", "authors", mode="before")
    @classmethod
    ...
    @classmethod
    def convert_target_audience_from_list(cls, v):
        """Convert list to comma-separated string (AI sometimes returns list)."""
        if isinstance(v, list):
            return ", ".join(str(item) for item in v if item)
        return v or ""

    @field_validator("voice_persona", mode="before")
    @classmethod
    def convert_voice_persona_from_string(cls, v):
        """Convert string to VoicePersona (AI sometimes returns simple string)."""
        if isinstance(v, str):
            return VoicePersona(voice_style=v)
        if isinstance(v, dict):
            return VoicePersona(**v)
        return v or VoicePersona()

    @field_validator("visual_identity", mode="before")
    @classmethod
    def convert_visual_identity_from_string(cls, v):
        """Convert string to VisualIdentity (AI sometimes returns simple string)."""
        if isinstance(v, str):
            return VisualIdentity(visual_style=v)
        if isinstance(v, dict):
            return VisualIdentity(**v)
        return v or VisualIdentity()
