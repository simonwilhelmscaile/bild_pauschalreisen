...
class Source(BaseModel):
    """A citation source with title, URL, and description."""
    title: str = Field(..., description="Source title or publication name (e.g. 'Gartner – Top Cybersecurity Trends 2025')")
    url: str = Field(..., description="Source URL")
    description: str = Field(..., description="1-2 sentence description explaining what this source provides or its key insights (e.g. 'Industry analysis outlining key cybersecurity risks and enterprise priorities for 2025')")
    ...
    def validate_description(cls, v):
        ...
        if not v or not v.strip():
            raise ValueError("Source description cannot be empty - must explain what the source provides")
        if len(v.strip()) < 10:
            raise ValueError("Source description too short - must be at least 10 characters explaining the source's contribution")
        return v.strip()
...
class ArticleOutput(BaseModel):
    ...
    Sources: List[Source] = Field(
        ...
        description="List of 3-5 high-quality citation sources. Each source must include: title (source name), url (source link), and description (1-2 sentences explaining what the source provides, e.g. 'Industry analysis outlining key cybersecurity risks and enterprise priorities for 2025' or 'Annual global report with empirical data on breach costs and security investments')"
    )
    ...
    @classmethod
    def convert_search_queries_from_list(cls, v):
        """Convert list to comma-separated string (AI sometimes returns list)."""
    ...
    def convert_sources_from_dicts(cls, v):
        ...
        Salvages sources with missing descriptions by auto-generating one
        from the title, rather than dropping them entirely.
        """
        if not v:
            return []
        result = []
        for item in v:
            if isinstance(item, dict):
                ...
                if url and url.strip():
                    # Auto-fill missing or empty description to prevent dropping valid sources
                    title = item.get("title", "")
                    desc = item.get("description", "")
                    if not desc or not desc.strip() or len(desc.strip()) < 10:
                        if title and title.strip():
                            item["description"] = f"Reference source: {title.strip()}"
                        else:
                            item["description"] = f"Reference source from {url.strip()}"
                        logger.info(f"Auto-filled missing description for source: {url[:80]}")
                    try:
                ...
                result.append(item)
        if not result and v:
            logger.warning(f"All {len(v)} sources were invalid and dropped")
        return result
