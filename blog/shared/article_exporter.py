...
class ArticleExporter:
    ...
    def _embed_images_for_pdf(html_content: str, base_path: Path) -> str:
        """
        Convert image references to base64 data URLs for PDF generation.
        
        Downloads external images (http/https) and converts local images to base64.
        
        Args:
            html_content: HTML content with image paths or URLs
        ...
        def replace_image_src(match):
            ...
            # Handle external URLs (http/https) - download and embed
            if src.startswith(('http://', 'https://')):
                try:
                    logger.debug(f"Downloading image for PDF: {src[:80]}...")
                    response = requests.get(src, timeout=15)
                    response.raise_for_status()
                    img_data = response.content
                    
                    if not img_data:
                        logger.debug(f"Downloaded image is empty: {src[:80]}")
                        return img_tag
                    
                    # Determine MIME type from URL or content-type header
                    content_type = response.headers.get('Content-Type', 'image/png')
                    if 'webp' in src.lower() or 'webp' in content_type:
                        mime_type = 'image/webp'
                    elif 'png' in src.lower() or 'png' in content_type:
                        mime_type = 'image/png'
                    elif 'jpg' in src.lower() or 'jpeg' in src.lower() or 'jpeg' in content_type:
                        mime_type = 'image/jpeg'
                    elif 'gif' in src.lower() or 'gif' in content_type:
                        mime_type = 'image/gif'
                    else:
                    ...
                    logger.info(f"Embedded external image for PDF: {src[:60]}... ({len(img_data)} bytes)")
                    return new_img_tag
                    
                except Exception as e:
                    logger.warning(f"Failed to download image for PDF: {src[:60]}... - {e}")
                    return img_tag
            ...
            if Path(src).is_absolute():
                ...
                if not img_path.exists():
                    logger.debug(f"Absolute image path not found for PDF: {src}")
                    return img_tag
            else:
            ...
            try:
                ...
                if not img_data:
                    logger.debug(f"Image file is empty: {src}")
                    return img_tag
