...
try:
    from services.blog.shared.field_utils import iter_content_fields
except ImportError:
    try:
        from shared.field_utils import iter_content_fields
    except ImportError as e:
        import warnings
        warnings.warn(f"Could not import field_utils: {e}")
        iter_content_fields = None

# Import replacement validator
try:
    from services.blog.shared.replacement_validator import validate_replacement
except ImportError:
    try:
        from shared.replacement_validator import validate_replacement
    except ImportError:
        validate_replacement = None
...
class QualityFixer:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        logger.debug("QualityFixer initialized (using core GeminiClient)")
    ...
    async def _get_fixes(
        ...
        try:
            ...
            # Use core GeminiClient with structured schema (no grounding needed)
    ...
    def _apply_fixes(
        ...
        for i, fix_data in enumerate(fixes):
            ...
            if validate_replacement is not None:
                pos = content_str.index(find_text)
                ctx_before = content_str[max(0, pos - 80):pos]
                ctx_after = content_str[pos + len(find_text):pos + len(find_text) + 80]
                is_html = field.startswith("section_") and field.endswith("_content")
                ok, rejection_reason = validate_replacement(
                    find_text, replace_text, ctx_before, ctx_after, is_html=is_html
                )
                if not ok:
                    preview = find_text[:50] + ('...' if len(find_text) > 50 else '')
                    logger.warning(f"  Rejected fix in {field}: {preview} — {rejection_reason}")
                    continue
