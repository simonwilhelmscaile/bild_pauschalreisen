...
try:
    ...
    print("[stage4/url_verifier] Using services.blog.shared.prompt_loader")
except ImportError as e:
    print(f"[stage4/url_verifier] Absolute import failed: {e}")
    try:
        # Fallback for relative import
        from ..shared.prompt_loader import load_prompt
        _PROMPT_LOADER_AVAILABLE = True
        print("[stage4/url_verifier] Using relative ..shared.prompt_loader")
    except ImportError as e2:
        print(f"[stage4/url_verifier] Relative import failed: {e2}")
        try:
            # Last fallback for direct script execution
            from shared.prompt_loader import load_prompt
            _PROMPT_LOADER_AVAILABLE = True
            print("[stage4/url_verifier] Using shared.prompt_loader (fallback)")
        except ImportError:
            _PROMPT_LOADER_AVAILABLE = False
            print("[stage4/url_verifier] No prompt_loader available")
...
class URLVerifier:
    """
    AI-powered URL verification using core GeminiClient.
    ...
    TIMEOUT = 30  # Reduced from 60 for faster generation
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        logger.info("URLVerifier initialized (using core GeminiClient)")
    ...
    async def find_replacements_batch(
        ...
        try:
            ...
            grounding_sources = result.get("_grounding_sources", [])
            grounding_urls = {}  # url -> title
            for source in grounding_sources:
                if source.get("url"):
                    grounding_urls[source["url"]] = source.get("title", "")

            if grounding_urls:
                logger.info(f"Found {len(grounding_urls)} real URLs from Google Search grounding")
            else:
                logger.warning("No grounding sources found - will not use AI-generated URLs (likely hallucinated)")
            ...
            for item in result.get("replacements", []):
                ...
                if old_url and new_url:
                    # Strict validation: only accept URLs that appear in grounding sources
                    # If no grounding sources, reject all AI-generated URLs (they're hallucinated)
                    if new_url not in grounding_urls:
                        if grounding_urls:
                            logger.debug(f"Skipping hallucinated URL (not in grounding): {new_url[:60]}...")
                        continue

                    replacements[old_url] = {
                        "new_url": new_url,
                        "source_name": item.get("source_name", "") or grounding_urls.get(new_url, ""),
                        "anchor_text": item.get("anchor_text", ""),
                        "reason": item.get("reason", "")
                    }

            # If AI suggestions didn't match grounding, use grounding sources directly
            if not replacements and grounding_urls and urls_to_process:
                logger.info("AI replacements didn't match grounding - using grounding sources directly")
                grounding_list = list(grounding_urls.items())
                for i, old_url in enumerate(urls_to_process):
                    if i < len(grounding_list):
                        new_url, title = grounding_list[i]
                        replacements[old_url] = {
                            "new_url": new_url,
                            "source_name": title,
                            "anchor_text": title or "source",
                            "reason": "From Google Search grounding"
                        }
...
- Rewritten sentence must be complete and grammatically correct — never return truncated or partial text
- Rewritten sentence must be similar in length to the original (no drastic shortening)
- Never return an empty or near-empty rewrite"""
