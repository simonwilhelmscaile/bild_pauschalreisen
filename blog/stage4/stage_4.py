...
async def run_stage_4(input_data: Stage4Input) -> Stage4Output:
    ...
    # Treat homepage redirects as dead (alive but useless — landed on site root)
    homepage_redirect_urls = [r.url for r in http_results if getattr(r, 'is_homepage_redirect', False)]
    if homepage_redirect_urls:
        alive_urls = [u for u in alive_urls if u not in set(homepage_redirect_urls)]
        dead_urls = dead_urls + homepage_redirect_urls
        logger.info(f"  Homepage redirects detected: {len(homepage_redirect_urls)} (treated as dead)")
                ...
                try:
                    ...
                    for removal in html_removals:
                        ...
                        if rewritten and original_sentence in content:
                            # Validate rewrite before applying
                            rewrite_ok = True
                            try:
                                from services.blog.shared.replacement_validator import validate_replacement as _rv
                                pos = content.index(original_sentence)
                                ctx_b = content[max(0, pos - 80):pos]
                                ctx_a = content[pos + len(original_sentence):pos + len(original_sentence) + 80]
                                ok, reason = _rv(original_sentence, rewritten, ctx_b, ctx_a, is_html=True)
                                if not ok:
                                    logger.warning(f"    Rejected rewrite for {url[:50]}...: {reason}")
                                    rewrite_ok = False
                            except ImportError:
                                pass
                            ...
                            if rewrite_ok:
                                ...
                                removed_count += 1
                                logger.info(f"    Stripped (rewrite rejected): {url[:50]}... from {field}")
                        else:
                            # Fallback: just strip the link
                            article[field] = smart_remove_dead_url(content, field, url)
                            removed_count += 1
