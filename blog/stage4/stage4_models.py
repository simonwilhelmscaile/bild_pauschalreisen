...
class Stage4Input(BaseModel):
    ...
    # Options - defaults optimized for faster generation
    ...
    timeout_seconds: float = Field(default=3.0, description="HTTP request timeout")
    verify_content: bool = Field(default=False, description="Whether to verify content relevance with url_context (slow)")
    find_replacements: bool = Field(default=True, description="Whether to find replacements for dead URLs")
    replace_irrelevant: bool = Field(default=False, description="Whether to also replace irrelevant URLs (requires verify_content)")
    verify_replacement_urls: bool = Field(default=False, description="Whether to HTTP-check replacement URLs before using")
    remove_dead_urls: bool = Field(default=True, description="Whether to remove dead URLs when no replacement found")
    max_urls_per_batch: int = Field(default=5, description="Max URLs to process per AI batch call")
    max_content_verify: int = Field(default=5, description="Max URLs to verify content relevance")
    max_concurrent_http: int = Field(default=20, description="Max concurrent HTTP checks")
